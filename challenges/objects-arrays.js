...
const uni = (arr,query) => {
  return arr.filter(el => el.toUpperCase().indexOf(query.toUpperCase()) !== -1);
};
console.log(uni(universities,'Uni'));
...
zooAnimals.map((name) => {
  names.push(name.animal_name);
})

const lowerCase = [names.map(animal_name => animal_name.toLowerCase())];
...
*/
 const lowerPopulation = [zooAnimals.filter(function(popu){
...
zooAnimals.forEach(function(popu) {
  populationTotal.push(popu.population);
})
